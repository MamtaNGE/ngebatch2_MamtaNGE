...
const queryUser = (personName: string): Promise<User> =>
  new Promise((resolve, reject) => {
    const result = userData.find((user) => user.name === personName);
    if (result) {
      resolve(result);
    } else {
      reject(new Error(`User '${personName}' not found`));
    }
  });
...
/*const queryFood = (foodId: number | null): Promise<Food> =>
  ...
  const queryFood = (foodId: number | null): Promise<Food> =>
  new Promise((resolve, reject) => {
    const result = foodData.find((food) => food.id === foodId);
    if (result) {
      resolve(result);
    } else {
      reject(new Error(`Food with ID '${foodId}' not found`));
    }
  });
