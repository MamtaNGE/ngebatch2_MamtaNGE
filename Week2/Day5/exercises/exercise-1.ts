...
const queryUser = (personName: string): Promise<User | null> => {
  return new Promise((resolve, reject) => {
    const result = userData.find((user) => user.name === personName) || null;
    resolve(result);
  });
};
...
const queryUser = (personName: string): Promise<User | null> => {
  return Promise.resolve(userData.find((user) => user.name == personName) || null);
};
...
const queryFood = (foodId: number): Promise<Food | null> => {
  return Promise.resolve(foodData.find((food) => food.id == foodId) || null);
};
